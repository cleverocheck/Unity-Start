...
public class StoreController : MonoBehaviour {
    ...
    private void Start() {
        // строковые литерылы это зло. исползуй тип а не название типа. плюс в том что не нужно импользовать GetComponent
        //  cubes = GameObject.Find("GlobalController").GetComponent<GlobalController>().cubes;
        cubes = FindObjectOfType<GlobalController>().cubes; 
        cubes_price = FindObjectOfType<GlobalController>().cubes_price;
        // два раза искать на сене обьект да и ище в одном месте. перемеая удалится как только завершится метод Start
        var globalController = FindObjectOfType<GlobalController>(); // так лучше. но тогда поля cubes и cubes_price не нужны 
        cubes = globalController.cubes; 
        cubes_price = globalController.cubes_price;
        ...
        for (int i = 0; i < cubes.Length; i++) {
            ...
            new_text.transform.localScale = Vector3.one; // new Vector3(1, 1, 1);
